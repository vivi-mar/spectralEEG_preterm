...
[FN, PN]  = uigetfile('/*******/*.edf', 'select .EDF file to extract subject DATA', 'multiselect', 'on');  
...
EEG.bipo.FC_SX=double(EEG.data(1,:)-EEG.data(4,:));
EEG.bipo.FT_SX=double(EEG.data(1,:)-EEG.data(3,:));
EEG.bipo.CO_SX=double(EEG.data(4,:)-EEG.data(7,:));
EEG.bipo.TO_SX=double(EEG.data(3,:)-EEG.data(7,:));
EEG.bipo.FC_DX=double(EEG.data(2,:)-EEG.data(5,:));
EEG.bipo.FT_DX=double(EEG.data(2,:)-EEG.data(6,:));
EEG.bipo.CO_DX=double(EEG.data(5,:)-EEG.data(8,:));
EEG.bipo.TO_DX=double(EEG.data(6,:)-EEG.data(8,:));
...
%% Plot EEG
eegplot(EEG.signal, 'srate', EEG.srate, 'winlength', 60)
...
for kk = 1:size(EEG.signal, 1)
    waitbar(kk/size(EEG.signal, 1))
...
if size(tempch,2) == EEG.nbchan
    eegplot(EEG.signal, 'winlength', 60)
...
EEGtemp.nbchan      = size(EEG.signal, 1);
EEGtemp.data        = EEG.signal;
EEGtemp.chanlocs    = EEG.chanlocs;
EEGtemp.trials      = size(EEG.signal, 3);
EEGtemp.pnts        = size(EEG.signal, 2);
